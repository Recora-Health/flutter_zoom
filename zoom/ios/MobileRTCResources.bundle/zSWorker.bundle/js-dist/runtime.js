...
function graftedFetch(request, opts) {
    //resolve cors problem: origin/url scheme protocol diffrence
    //docs has fetch navigator.sendBeacon axios/xhr request method
    if(request === undefined){
        return originalFetch(request, opts);
    }
    var url = request instanceof Request ? request.url : request;
    if(url.includes('/zoomdoc/')){
        opts.headers = opts.headers || {};
        opts.headers['isAPI'] = true;
    }
    if(url.includes('/api/') || url.includes('/nws/common/')){
        opts.headers = opts.headers || {};
        opts.headers['isAPI'] = true;
...
var originalSetRequestHeader_1 = XMLHttpRequest.prototype.setRequestHeader;
const uploadMap = new Map();

function PatchXHRState(xhr, state) {
  Object.defineProperty(xhr, 'readyState', {
    ...
    xhr.getResponseHeader = (key) => {
        return state.headers[key.toLowerCase()] || null;
    };
...
XMLHttpRequest.prototype.open = function (method, url) {
    ...
    this._url = url;
    this._headers = {};
    return originalOpen_1.apply(this, arguments);
};
    
XMLHttpRequest.prototype.setRequestHeader = function(key, value) {
    if (!this._headers) this._headers = {};
    this._headers[key] = value;
    return originalSetRequestHeader_1.apply(this, arguments);
};
...
XMLHttpRequest.prototype.send = function (data) {
      if (data instanceof FormData) {
        ...
        Promise.all(fileReadPromises).then((files) => {
            apiRequest("/XMLHttpRequest/postFormData", {
                uploadId: uploadId,
                url: this._url,
                method: this._method,
                ...
                console.error("XMLHttpRequest to postFormData formData", err);
            });
        });
        this.setRequestHeader('X-Upload-ID', uploadId);
        const internalState = {
            status: 0,
            statusText: '',
            responseText: '',
        ...
        PatchXHRState(this, internalState);
        uploadMap.set(uploadId, this);
        
      if(this.signal){
        const onAbort = () => {
            simulateAbort(xhr, uploadId);
            uploadMap.delete(uploadId);
        };
        this.signal.aborted ? onAbort() : this.signal.addEventListener('abort', onAbort);
      }
    ...
    this.setRequestHeader('isAPI', true);
    this.withCredentials = true;
    return originalSend_1.apply(this, arguments);
};

    //navigator.sendBeacon
const originalSendBeacon = navigator.sendBeacon;
navigator.sendBeacon = function(url, data) {
//    if (url.includes("block-this-request")) {
//        console.warn("Blocked sendBeacon request:", url);
//        return false;
//    }
    return true;
...
// uploading
window.onSWNativeUploadProgress = function (progress) {
  const { uploadId, loaded, total, percent } = progress || {};
  console.log(`[progress] ${uploadId}: ${percent}% (${loaded}/${total})`);

  const xhr = uploadMap.get(uploadId);
  if (!xhr) return;
...
// uploadfinished
window.onSWNativeUploadFinished = function (result) {
  const {
    uploadId,
    status = 200,
    headers = {},
  ...
  console.log(`[finished] ${uploadId}`, result);

  const xhr = uploadMap.get(uploadId);
  if (!xhr) return;
  uploadMap.delete(uploadId);

  const state = xhr.__internalState;
  state.status = error ? (status || 500) : status;
  state.statusText = error ? (error.message || "Error") : "OK";
  state.responseText = responseText;
  state.readyState = 4;
  state.headers = headers;

  if (typeof xhr.onreadystatechange === "function") xhr.onreadystatechange();
  if (!error && typeof xhr.onload === "function") xhr.onload();
  if (error && typeof xhr.onerror === "function") xhr.onerror(new Error(responseText));
  if (error?.code === -1 && typeof xhr.onabort === "function") xhr.onabort();
};

...
#import <MobileRTC/MobileRTCMeetingChat.h>
#import <MobileRTC/MobileRTCAppSignalPanelHandler.h>
 ...
 * @param reason The reason why the meeting has ended.
 */
 ...
 * @note The new join user can not join meeting, but they can watch the meeting live stream.
 */
...
/**
 * @brief Callback event when additional information is required to join the meeting.
 * @param handler The handler to input the required information or cancel joining the meeting.
 */
 ...
 * @deprecated Use -onJoinMeetingInfoRequired: instead.
 */
 ...
 * @param type if the type is [FreeMeetingNeedUpgradeType_BY_GIFTURL], the user can upgrade through the url. If the type is [FreeMeetingNeedUpgradeType_BY_ADMIN], only the user's admin can upgrade the meeting.
 ...
 * @return YES: The developer takes a custom action and the SDK does nothing. NO: The SDK still takes its default action.
 * @note If user wants to custom Share Action Item, add items to Share ActionSheet via MobileRTCMeetingShareActionItem. Otherwise YES, the user uses the default UI.
 */
...
/**
 * @brief Determines if the meeting VoIP call is running. It affects the picture-in-picture function in ZoomUI if videoCallPictureInPictureEnabled returns YES of MobileRTCMeetingSettings.
 * @return YES if the meeting VoIP call is running. Otherwise, NO.
 */
 ...
 * @param state The new paring state.
 ...
 * @param deleteBy Information about who deleted the message.
 */
 ...
 * @param liveStreamStatus The new status.
 */
...
/**
 * @brief Callback event when the live stream reminder enable status changed.
 * @param enable true means the live stream reminder is enabled.
 */
- (void)onLiveStreamReminderStatusChanged:(BOOL)enable;

/**
 * @brief Callback event when the live stream reminder enable status change failed.
 */
- (void)onLiveStreamReminderStatusChangedFailed;

/**
 * @brief Callback event when the meeting/webinar user has nearly reached the meeting capacity, like 80% or 100% for the meeting/webinar capacity.
 * @param percent proportion of joined users to the total capacity.
 * @note The host can start live stream to let unjoined users watch live stream.
 */
...
/**
 * @brief Callback event when participants are allowed or disallowed to start video.
 * @param allow YES to allow, NO to disallow.
 */
- (void)onAllowParticipantsStartVideoNotification:(BOOL)allow;

/**
 * @brief Callback event when participants are allowed or disallowed to rename themselves.
 * @param allow YES to allow. If NO, participants may not rename themselves.
 */
- (void)onAllowParticipantsRenameNotification:(BOOL)allow;

/**
 * @brief Callback event when participants are allowed or disallowed to unmute themselves.
 * @param allow YES to allow. If NO, participants may not unmute themselves.
 */
- (void)onAllowParticipantsUnmuteSelfNotification:(BOOL)allow;

/**
 * @brief Callback event when participants are allowed or disallowed to share a new whiteboard.
 * @param allow YES to allow. If NO, participants may not share new whiteboards.
 */
- (void)onAllowParticipantsShareWhiteBoardNotification:(BOOL)allow;

/**
 * @brief Callback event when participants are allowed or disallowed to share.
 * @param allow YES to allow. If NO, participants may not share.
 */
- (void)onAllowParticipantsShareStatusNotification:(BOOL)allow;
/**
 * @brief Callback event when the meeting lock status changes.
 * @param isLock YES if the status is locked. Otherwise, NO.
 */
...
/**
 * @brief Callback event when participant profile status changes.
 ...
 */
- (void)onAppSignalPanelUpdated:(MobileRTCAppSignalPanelHandler* _Nullable)handler;

#pragma mark - MobileRTC Recording Delegate -

/**
 * @brief Determines if the current user has cloud recording privileges.
 ...
 * @deprecated Use MobileRTCAICompanionSmartSummaryHelperDelegate instead.
 */
...
/**
 * @brief A user's audio type has changed. To get their updated type, check \link MobileRTCMeetingUserInfo::audioStatus \endlink for the associated user.
...
/**
 * @brief A participant's video status has changed. To get their updated status, check \link MobileRTCMeetingUserInfo.videoStatus \endlink for the associated user. Valid for both normal user and webinar attendee.
 ...
 * @warning When this is invoked, any instances of \link MobileRTCPreviewVideoView \endlink no longer render video and should be removed.
 */
 ...
 * @deprecated Use onActiveSpeakerVideoUserChanged: instead.
 */
 ...
 * @deprecated Not maintain anymore,Use onSinkMeetingUserRaiseHand or onSinkMeetingUserLowerHand instead.
 */
 ...
 * @deprecated Will delete later, if you need this API, please contact us.
 */
 ...
 * @param availableSignLanguageList Specify the available sign languages list.
 */
...
/**
 * @brief Sink the event to start captions.
...
/**
 * @brief Sink the event to start captions was approved.
 */
 ...
 * @deprecated Use onCreateBOResponse:BOID: instead.
 */
...
#pragma mark - Recover meeting  -
/**
 * @brief Callback event when host start a deleted or expired meeting(not PMI meeting).
 ...
 * @param featuresArr The AI features that can't be turned off.
 */
...
#pragma mark - MobileRTCDoc

 /**
 * @brief Docs status changed callback. This function is used to inform the user docs has been started or stopped, and all users in the meeting can get the event.
 * @param sourceInfo docs source info.
  */
 ...
 * @brief The class that conform to the MobileRTCCustomizedUIMeetingDelegate protocol can provide methods for tracking the In-Meeting Event and determining policy for each event.
 * @note The MobileRTCCustomizedUIMeetingDelegate protocol is required in the custom meeting UI view.
 */
 ...
 * @note The MobileRTCVideoRawDataDelegate protocol is required in the custom meeting UI view.
 */
 ...
 * @note The MobileRTCAudioRawDataDelegate protocol is required in the custom meeting UI view.
 */
 ...
 */
- (void)onShareAudioRawDataReceived:(MobileRTCAudioRawData *_Nonnull)rawData userId:(NSUInteger)userId;
@end
 ...
 * @note The MobileRTCAudioSourceDelegate protocol is required in the custom meeting UI view.
 */
 ...
 * @note The MobileRTCPreProcessorDelegate protocol is required in the custom meeting UI view.
 */
 ...
 * @note The MobileRTCVideoSourceDelegate protocol is required in the custom meeting UI view.
 */
 ...
 * @param rawDataSender MobileRTCVideoSender.
...
#pragma mark - MobileRTCMeetingSettingsDelegate
/**
 * @protocol MobileRTCMeetingSettingsDelegate
 * @brief Delegate protocol for meeting settings events, including 3D avatar settings.
 */
@protocol MobileRTCMeetingSettingsDelegate <NSObject>
@optional

/**
 * @brief Notification of whether or not the custom 3d avatar image model data has been downloaded successfully.
 * @param success YES means the custom 3d avatar image model data has been downloaded successfully.
 * @param imageInfo The image info of the downloaded custom 3d avatar image.
 */
- (void)onCustom3DAvatarImageModelDataDownloaded:(BOOL)success imageInfo:(MobileRTC3DAvatarImageInfo * _Nullable)imageInfo;

/**
 * @brief Notification of whether or not the custom 3d avatar default image model data has been downloaded successfully.
 * @param success YES means the custom 3d avatar default image model data has been downloaded successfully.
 */
